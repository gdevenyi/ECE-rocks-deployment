...
#$ -S /bin/bash
#Specify the archetecture (linux-x86 or linux-x64, remove this line to allow any machien)
#$ -l arch=linux-x86
...
#Note that the matlab program will decide to run 32 or 64bit based on which node it runs on
matlab -nodisplay -nodesktop < myscriptfile.m
